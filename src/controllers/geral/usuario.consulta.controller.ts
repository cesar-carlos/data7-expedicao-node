...
export default class UsuarioConsultaController {
  ...
  public static post = handleController(async (_req: Request, res: Response): Promise<void> => {
    res.status(405).send({
      message: 'Método POST não permitido para consulta de usuários. Use GET.',
    });
  });

  public static put = handleController(async (_req: Request, res: Response): Promise<void> => {
    res.status(405).send({
      message: 'Método PUT não implementado para consulta de usuários.',
    });
  });

  public static delete = handleController(async (_req: Request, res: Response): Promise<void> => {
    res.status(405).send({
