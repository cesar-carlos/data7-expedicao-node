...
export const consultaVersaoAppQuerySchema = z.object({
  NomeApp: z
    .string()
    .transform((val) => val.trim())
    .refine((val) => val.length > 0, 'Nome do App não pode estar vazio')
    .refine((val) => val.length <= 100, 'Nome do App deve ter no máximo 100 caracteres')
    .optional(),
...
export type ConsultaVersaoAppQuery = z.infer<typeof consultaVersaoAppQuerySchema>;
